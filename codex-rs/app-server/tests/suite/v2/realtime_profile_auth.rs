...
async fn remote_voice_uses_profile_auth(
    ...
    let sideband = start_websocket_server_with_headers(vec![WebSocketConnectionConfig {
        requests: Vec::new(),
    ...
    let selected_alias = match selection {
        ProfileSelection::Default | ProfileSelection::Unavailable => "alpha",
        ProfileSelection::Pinned
        | ProfileSelection::ChangedDefault
    ...
    if matches!(selection, ProfileSelection::Unavailable) {
        ...
        assert!(sideband.wait_for_handshakes(1, TIMEOUT).await);
        let handshake = sideband.single_handshake();
        ...
        if matches!(selection, ProfileSelection::ChangedDefault) {
            ...
            let removal: Result<AccountProfileRemoveResponse> = server
            ...
            assert!(
                removal.is_err(),
                "the active voice call must retain its profile lease"
            );
        }
