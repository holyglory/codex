...
pub(super) fn paginate(value: &mut Value, args: &UsageStatsArgs) -> Result<(), FunctionCallError> {
    ...
    if args.cursor_id.is_some() != args.cursor_sort_value.is_some() {
        return Err(tool_error(
            "summary cursor fields must be supplied together",
        ));
    }
    let encoded =
        serde_json::to_vec(value).map_err(|_| tool_error("cannot encode usage summary"))?;
    ...
    let cursor_id = format!(
        "summary-v1-{:x}",
        Sha1::digest(identity.to_string().as_bytes())
    );
    ...
    for &field in fields {
        let values = value[field]
            .as_array_mut()
            .ok_or_else(|| tool_error("invalid summary details"))?;
    ...
    loop {
        *value = base.clone();
        ...
        if serde_json::to_vec(value)
            .map_err(|_| tool_error("cannot encode usage summary"))?
            .len()
            <= budget
        {
            return Ok(());
        ...
        if end == offset {
            return Err(tool_error(
                "summary totals exceed the safe bound; narrow the time range",
            ));
        }
        end -= 1;
