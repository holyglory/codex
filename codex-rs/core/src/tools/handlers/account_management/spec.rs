...
pub(super) fn account_management_spec() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Manage existing local account profiles and inspect current service limits. Use list before mutations and supply its expected_generation. Rename, enable/disable, choose the default profile, configure automatic selection, or change priorities. Changes preserve the current turn's credential lease and apply to subsequent routing. With refresh_service_usage, list returns live limit windows, Unix reset timestamps and the next reset date in UTC. Paginate to inspect every account. This tool never returns credentials, email, service/workspace identifiers or notes, and does not add profiles, perform login or remove profiles. Background account probes remain limited to managed ChatGPT profiles."
            .to_string(),
        strict: false,
        defer_loading: None,
