...
async fn optional_mcp_startup_grace_controls_initial_turn_tool_catalog(
    ...
    let startup_timeout = match scenario {
        // Include real connection setup before reaching the gated initialize request.
        StartupGraceScenario::DisabledGraceRespectsStartupTimeout => Duration::from_secs(2),
